 ...
 * ----------------------------
 * Mengatur tampilan produk di sisi customer:
 * - Halaman beranda
...
class ProductController extends Controller
{
     ...
     * - Teks & statistik hero (dari SiteSetting)
     * - Slide hero (gambar carousel)
     */
    public function home()
    {
        // Ambil 6 kategori + hitung berapa produk di masing-masing kategori
        $categories = Category::withCount('products')->take(6)->get();

        // Ambil 8 produk aktif terbaru (pakai scope active())
        $featuredProducts = Product::active()->latest()->take(8)->get();

        // Ambil semua teks/statistik hero dari tabel site_settings
        // Parameter kedua = nilai default jika key belum ada di database
        $home = [
            ...
            // 4 poin kepercayaan (trust badge)
            'trust' => [
                [
                    'title' => SiteSetting::get('trust_1_title', 'Pengiriman Cepat'),
                    'desc'  => SiteSetting::get('trust_1_desc', 'Ke seluruh Indonesia'),
                    'icon'  => 'M8.25 18.75a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 01-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h1.125c.621 0 1.129-.504 1.09-1.124a17.902 17.902 0 00-3.213-9.193 2.056 2.056 0 00-1.58-.86H14.25M16.5 18.75h-2.25m0-11.177v-.958c0-.568-.422-1.048-.987-1.106a48.554 48.554 0 00-10.026 0 1.106 1.106 0 00-.987 1.106v7.635m12-6.677v6.677m0 4.5v-4.5m0 0h-12',
                ],
                [
                    'title' => SiteSetting::get('trust_2_title', '100% Original'),
                    'desc'  => SiteSetting::get('trust_2_desc', 'Produk bergaransi'),
                    'icon'  => 'M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z',
                ],
                [
                    'title' => SiteSetting::get('trust_3_title', 'Bayar Aman'),
                    'desc'  => SiteSetting::get('trust_3_desc', 'Transfer, e-wallet, COD'),
                    'icon'  => 'M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5z',
                ],
                [
                    'title' => SiteSetting::get('trust_4_title', 'Mudah Retur'),
                    'desc'  => SiteSetting::get('trust_4_desc', '7 hari jaminan'),
        ...
        // Ambil slide hero yang aktif, diurutkan
    ...
    {
        // Mulai query: hanya produk aktif + load relasi kategori
        ...
        // Jika ada parameter ?category=slug-kategori
        ...
        $query->when($request->sort === 'price_asc',  fn ($q) => $q->orderBy('price', 'asc'))
              ...
              ->when(!$request->filled('sort'),       fn ($q) => $q->latest()); // default: terbaru

        // Ambil data + pagination, tetap bawa query string (search, category, sort)
        $products = $query->paginate(12)->withQueryString();

        // Semua kategori untuk filter sidebar
     ...
     * --------------------------------------
     * - Menambah views_count
     * - Load gambar & kategori
     * - Ambil 4 produk sejenis (related)
     */
    public function show(Product $product)
    {
        // Route model binding otomatis mencari produk berdasarkan slug
        // Karena di route: {product:slug}

        // Tambah 1 hitungan view
        $product->incrementViews();

        // Load relasi images dan category (eager loading)
        $product->load('images', 'category');

        // Produk terkait: kategori sama, bukan produk ini sendiri, maksimal 4
