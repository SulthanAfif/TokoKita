{{-- Ikon lonceng notifikasi + dropdown status pesanan --}}
@auth
@php
...
<div class="relative" x-data="{ open: false }" @click.outside="open = false">
    ...
    <div x-show="open" x-cloak
        ...
        <div class="flex items-center justify-between px-4 py-3 border-b border-slate-100">
            ...
            <a href="{{ route('orders.index') }}" class="text-xs font-medium text-indigo-600 hover:text-indigo-500">Lihat Semua</a>
        </div>
            ...
            @foreach([
                ...
                <a href="{{ route('orders.index', ['status' => $st]) }}"
                   class="flex flex-col items-center gap-1 p-2 rounded-xl hover:bg-white transition text-center">
                    <span class="relative w-10 h-10 rounded-full {{ $statusIcons[$st]['color'] }} flex items-center justify-center">
                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="{{ $meta[1] }}" />
                        </svg>
                        @if(($counts[$st] ?? 0) > 0)
                    ...
                    <span class="text-[10px] text-slate-600 leading-tight">{{ $meta[0] }}</span>
                </a>
            @endforeach
        </div>
        ...
        <div class="max-h-64 overflow-y-auto divide-y divide-slate-50">
            @forelse($orders as $order)
                @php $si = $statusIcons[$order->status] ?? ['label' => $order->status, 'color' => 'bg-slate-100 text-slate-600']; @endphp
                <a href="{{ route('orders.show', $order) }}"
                   class="flex items-start gap-3 px-4 py-3 hover:bg-slate-50 transition">
                    <span class="mt-0.5 shrink-0 w-8 h-8 rounded-full {{ $si['color'] }} flex items-center justify-center">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <p class="text-xs text-slate-500">{{ $si['label'] }} · {{ $order->created_at->diffForHumans() }}</p>
                    </div>
                </a>
            @empty
