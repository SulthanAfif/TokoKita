...
<div class="max-w-2xl">
    <div class="mb-6">
        ...
        <h2 class="text-lg font-semibold text-slate-800 mt-2">Edit Produk</h2>
    ...
    <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data"
        ...
        <div>
            ...
            <textarea name="description" rows="4"
                      class="w-full rounded-xl border-slate-300 text-sm focus:ring-indigo-500 focus:border-indigo-500">{{ old('description', $product->description) }}</textarea>
        </div>
        ...
        @if($product->thumbnail)
            <div>
                ...
                <img src="{{ $product->thumbnail_url }}"
                     class="w-24 h-24 rounded-xl object-cover border border-slate-200" alt="">
            </div>
        @endif

        <div>
            <label class="block text-sm font-medium text-slate-700 mb-1">Ganti Foto Produk</label>
            ...
            <input type="url" name="thumbnail_url" value="{{ old('thumbnail_url') }}"
                   ...
                   class="w-full rounded-xl border-slate-300 text-sm focus:ring-indigo-500 focus:border-indigo-500">
            <p class="text-xs text-slate-400 mt-1">Tempel link gambar dari internet. Isi salah satu saja (upload file akan diprioritaskan kalau keduanya diisi). Biarkan kosong kalau tidak mau ganti foto.</p>
