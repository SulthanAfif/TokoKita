...
<section
    x-data="{
        slides: <?php echo e($heroSlides->map(fn($s) => ['url' => $s->image_url])->values()->toJson()); ?>,
        current: 0,
        timer: null,
        init() {
    ...
    <div class="relative grid lg:grid-cols-2 gap-8 items-center px-5 py-10 sm:px-10 sm:py-16 lg:px-12 lg:py-20">
        <div>
            <span class="inline-flex items-center gap-1.5 rounded-full bg-white/15 backdrop-blur px-3.5 py-1.5 text-xs font-semibold tracking-wide mb-5 border border-white/20">
                ...
                <?php echo e($home['hero_badge']); ?>

            </span>

            <h1 class="text-3xl sm:text-4xl md:text-5xl lg:text-[3.25rem] font-extrabold tracking-tight leading-[1.1]">
                <?php echo e($home['hero_title_1']); ?><br>
                <span class="bg-gradient-to-r from-yellow-200 via-pink-200 to-white bg-clip-text text-transparent"><?php echo e($home['hero_title_2']); ?></span>
            </h1>

            <p class="mt-5 text-indigo-100 max-w-md text-base sm:text-lg leading-relaxed">
                <?php echo e($home['hero_subtitle']); ?>
            ...
            <div class="mt-10 flex flex-wrap gap-6 sm:gap-10">
                <div>
                    <p class="text-2xl sm:text-3xl font-extrabold"><?php echo e($home['stat1_value']); ?></p>
                    <p class="text-xs text-indigo-200 mt-0.5"><?php echo e($home['stat1_label']); ?></p>
                </div>
                <div>
                    <p class="text-2xl sm:text-3xl font-extrabold"><?php echo e($home['stat2_value']); ?></p>
                    <p class="text-xs text-indigo-200 mt-0.5"><?php echo e($home['stat2_label']); ?></p>
                </div>
                <div>
                    <p class="text-2xl sm:text-3xl font-extrabold"><?php echo e($home['stat3_value']); ?></p>
                    <p class="text-xs text-indigo-200 mt-0.5"><?php echo e($home['stat3_label']); ?></p>
                </div>
            </div>
        </div>
        ...
        <div class="hidden lg:flex justify-center relative">
            <div class="relative w-72 h-72">
                ...
                <div class="absolute inset-0 rounded-[2rem] bg-white/15 backdrop-blur-sm border border-white/25 -rotate-3 flex flex-col items-center justify-center p-8">
                    ...
                    <p class="text-lg font-bold text-center"><?php echo nl2br(e($home['hero_card_title'])); ?></p>
                    <p class="text-sm text-indigo-100 mt-2 text-center"><?php echo e($home['hero_card_subtitle']); ?></p>
                </div>
                <div class="absolute -top-4 -right-4 bg-gradient-to-r from-pink-500 to-rose-500 text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-lg rotate-12">
                    <?php echo e($home['hero_badge_promo']); ?>
                ...
                <div class="absolute -bottom-3 -left-6 bg-white text-indigo-700 text-xs font-bold px-3 py-1.5 rounded-full shadow-lg -rotate-6">
                    <?php echo e($home['hero_badge_flash']); ?>
...
<section class="mt-8 grid grid-cols-2 lg:grid-cols-4 gap-3">
    <?php $__currentLoopData = $home['trust']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <div class="flex items-center gap-3 rounded-2xl bg-white border border-slate-100 p-4 shadow-sm hover:shadow-md hover:border-indigo-100 transition">
            ...
            <div>
                <p class="text-sm font-bold text-slate-800"><?php echo e($item['title']); ?></p>
                <p class="text-xs text-slate-400"><?php echo e($item['desc']); ?></p>
            </div>
        </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3 sm:gap-4">
        <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <a href="<?php echo e(route('products.index', ['category' => $category->slug])); ?>"
               class="group relative overflow-hidden rounded-2xl p-5 text-center text-white shadow-lg hover:shadow-xl hover:-translate-y-1 transition duration-300 bg-gradient-to-br <?php echo e($gradients[$i % count($gradients)]); ?>">
                ...
                <div class="relative">
                    ...
                    <div class="text-sm font-bold"><?php echo e($category->name); ?></div>
                    <div class="text-[11px] text-white/70 mt-0.5"><?php echo e($category->products_count); ?> produk</div>
                </div>
            </a>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    </div>
...
<section class="mt-14 relative overflow-hidden rounded-2xl bg-gradient-to-r from-rose-500 via-pink-500 to-orange-400 p-6 sm:p-8 text-white shadow-lg shadow-pink-500/20">
    ...
    <div class="relative flex flex-col sm:flex-row items-center justify-between gap-4">
        <div>
            <p class="text-xs font-bold uppercase tracking-widest text-white/80 mb-1"><?php echo e($home['promo_eyebrow']); ?></p>
            <h3 class="text-xl sm:text-2xl font-extrabold"><?php echo e($home['promo_title']); ?></h3>
            <p class="text-sm text-white/80 mt-1"><?php echo e($home['promo_subtitle']); ?></p>
        </div>
        <a href="<?php echo e(route('products.index')); ?>"
           class="shrink-0 rounded-full bg-white text-pink-600 font-bold px-6 py-3 hover:scale-105 shadow-lg transition">
            <?php echo e($home['promo_button']); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\proyekporto\project\TokoKita\resources\views/home.blade.php ENDPATH**/ ?>