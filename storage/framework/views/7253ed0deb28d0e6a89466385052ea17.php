...
<div class="max-w-lg mx-auto" x-data="{ processing: false, verified: false }">
    <a href="<?php echo e(route('orders.show', $order)); ?>" class="text-sm text-slate-500 hover:text-indigo-600 transition">← Kembali ke pesanan</a>

    <h1 class="text-2xl font-bold text-slate-800 mt-3 mb-1">Pembayaran</h1>
    <p class="text-sm text-slate-500 mb-6"><?php echo e($order->order_number); ?> · <?php echo e($paymentLabels[$order->payment_method] ?? ''); ?></p>

    
    <div class="rounded-2xl border border-slate-200 bg-white p-5 mb-5">
        <div class="flex justify-between text-sm mb-1">
            <span class="text-slate-500">Total Pembayaran</span>
            <span class="text-xl font-bold text-indigo-600">Rp<?php echo e(number_format($order->total, 0, ',', '.')); ?></span>
        </div>
        <p class="text-xs text-slate-400"><?php echo e($order->items->count()); ?> item</p>
    </div>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-5 mb-5 space-y-3 text-sm">
        ...
        <div class="rounded-xl bg-slate-50 p-4 space-y-2">
            ...
            <div class="flex justify-between"><span class="text-slate-500">Jumlah Transfer</span><span class="font-bold text-indigo-600">Rp<?php echo e(number_format($order->total, 0, ',', '.')); ?></span></div>
        </div>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-5 mb-5 space-y-3 text-sm">
        ...
        <div class="rounded-xl bg-slate-50 p-4 space-y-2">
            ...
            <div class="flex justify-between"><span class="text-slate-500">Jumlah</span><span class="font-bold text-indigo-600">Rp<?php echo e(number_format($order->total, 0, ',', '.')); ?></span></div>
        </div>
    ...
    <div x-show="!processing && !verified">
        <form action="<?php echo e(route('orders.processPayment', $order)); ?>" method="POST"
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\proyekporto\project\TokoKita\resources\views/orders/payment.blade.php ENDPATH**/ ?>