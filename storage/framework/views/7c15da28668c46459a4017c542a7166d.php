...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
    <div>
        ...
        <p class="text-sm text-slate-500"><?php echo e($products->total()); ?> produk terdaftar</p>
    </div>
                            ...
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-lg bg-slate-100 overflow-hidden shrink-0">
                                    <?php if($product->thumbnail): ?>
                                        <img src="<?php echo e($product->thumbnail_url); ?>" class="w-full h-full object-cover" alt="">
                                    <?php endif; ?>
                                </div>
                                <div class="min-w-0">
                                    <p class="font-medium text-slate-800 truncate"><?php echo e($product->name); ?></p>
                                    <p class="text-xs text-slate-400"><?php echo e($product->sku); ?></p>
                                </div>
                            </div>
                        </td>
                        <td class="px-5 py-3.5 text-slate-600"><?php echo e($product->category->name ?? '-'); ?></td>
                        <td class="px-5 py-3.5">
                            <p class="font-semibold">Rp<?php echo e(number_format($product->final_price, 0, ',', '.')); ?></p>
                            <?php if($product->has_discount): ?>
                                <p class="text-xs text-slate-400 line-through">Rp<?php echo e(number_format($product->price, 0, ',', '.')); ?></p>
                            <?php endif; ?>
                        </td>
                        <td class="px-5 py-3.5 font-semibold text-emerald-600"><?php echo e((int) ($product->sold_qty ?? 0)); ?></td>
                        <td class="px-5 py-3.5">
                            <span class="<?php echo e($product->stock <= 5 ? 'text-red-600 font-semibold' : 'text-slate-700'); ?>">
                                <?php echo e($product->stock); ?>

                            </span>
                        </td>
                        <td class="px-5 py-3.5">
                            <span class="px-2.5 py-0.5 rounded-full text-xs font-medium <?php echo e($product->is_active ? 'bg-green-50 text-green-700' : 'bg-slate-100 text-slate-500'); ?>">
                ...
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                    <tr>
                        <td colspan="7" class="px-5 py-12 text-center text-slate-400">
                            Belum ada produk.
                            <a href="<?php echo e(route('admin.products.create')); ?>" class="text-indigo-600 font-medium">Tambah sekarang →</a>
                        </td>
                    </tr>
                <?php endif; ?>
            </tbody>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\proyekporto\project\TokoKita\resources\views/admin/products/index.blade.php ENDPATH**/ ?>